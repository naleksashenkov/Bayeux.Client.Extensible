...
import * as nodeCometDClient from "cometd-nodejs-client";
import * as nodeCometDServer from "../cometd-nodejs-server.js";
...
describe("acknowledgment extension", () => {
    ...
    beforeEach(done => {
        nodeCometDClient.adapt();
        _server = nodeCometDServer.createCometDServer();
        _server.addExtension(new nodeCometDServer.AcknowledgedMessagesExtension());
        _http = http.createServer((request, response) => _server.handle(request, response));
        _http.listen(0, "localhost", () => {
            const port = _http.address().port;
            console.log("listening on localhost:" + port);
            _uri = "http://localhost:" + port + "/cometd";
            _client = new jsCometD.CometD();
            _client.unregisterTransport("websocket");
            _client.registerExtension("ack", new jsCometD.AckExtension());
            _client.configure({
                url: _uri
            });
            done();
    ...
    it("resends messages", done => {
        // Set up the client to fake the batch numbers.
