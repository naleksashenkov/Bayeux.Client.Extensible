...
describe("server", () => {
    ...
    beforeEach(done => {
        ...
        _handler = (request, response) => _server.handle(request, response);
        _http = http.createServer(_handler);
    ...
    function newRequest(headers) {
        // Could not find a way to make it work with TypeScript.
        const request = new URL(_uri);
        request.method = "POST";
    ...
    it("handles body already read", done => {
        // Replace the handler.
        _http.removeListener("request", _handler);
                ...
                for (let name in headers) {
                    if (headers.hasOwnProperty(name)) {
                        if (/^set-cookie$/i.test(name)) {
                            ...
                            for (let i = 0; i < values.length; ++i) {
                                let value = values[i];
                                if (value.indexOf("Partitioned") >= 0 && value.indexOf("Path=/") >= 0 && value.indexOf("Secure") >= 0) {
                                    done();
                                    return;
                                }
                            }
